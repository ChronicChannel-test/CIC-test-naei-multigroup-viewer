...
| `interaction` | `email_share_copied` | Email helper copies the chart image + opens mail client | Indicates highest-intent sharing workflow. |
| `interaction` | `data_export` | CSV/XLSX data export runs (`bubblechart/export.js`) | Includes format, pollutant, year, and category count for downstream aggregation. |
...
| `interaction` | `share_email_opened` | Email workflow launched | Logs pollutant/category context plus whether a year range was chosen. |
| `interaction` | `data_export` | Time-series CSV/XLSX exported | Records format, pollutant, category list, year range, and filename base. |
